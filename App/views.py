...
def ListFunc(request):
    ...
    return render(request,'App/templates/imgList.html',{'data':data})

def result_info(request):
    global context
    return render(request,'App/templates/results.html',context)
        ...
        if form1.is_valid():
            ...
            with torch.no_grad():
                for step, batch in enumerate(test_loader):  
                    ...
                    val_outputs_[x1:x1+192,y1:y1+192,z1:z1+96] = val_outputs[0,1,:,:,:].detach().cpu().numpy()        
                    ...
                    nib.save(tmp, os.path.join(dpath, 'your_nii_mask_SN.nii.gz').replace("\\", "/"))
